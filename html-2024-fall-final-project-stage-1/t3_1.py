...
# with pitcher
# LinearRegression : 0.5577747378020976
# LogisticRegression : 0.5569083447332421
# RandomForestClassifier : 0.5597355221158231



# without pitcher
# LinearRegression : 0.5571363429092567
# LogisticRegression : 0.5584587323301415
# RandomForestClassifier : 0.5595987232102144
...
for _ in range(REPT):
    ...
    X_train = data_process.pitcher_win_proce(X_train, p_win_rate)
    X_test = data_process.pitcher_win_proce(X_test, p_win_rate)
    # X_train = X_train.drop(columns=['home_pitcher','away_pitcher'])
    # X_test = X_test.drop(columns=['home_pitcher','away_pitcher'])

    X_train = X_train.drop(columns=['home_team_win'])
    X_test = X_test.drop(columns=['home_team_win'])

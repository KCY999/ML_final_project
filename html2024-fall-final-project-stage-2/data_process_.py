...
def pitcher_win_proce(data):
    with open('./processed_data/pitcher_win_rate.json', "r", encoding="utf-8") as json_file:
        team_win_rate = json.load(json_file)
    # 使用 map 映射勝率
    data['hp_win_rate'] = data['home_pitcher'].apply(lambda x: team_win_rate.get(x,0.5))
    data['ap_win_rate'] = data['away_pitcher'].apply(lambda x: team_win_rate.get(x,0.5))
    data["hp_ap_win_diff"] = data["hp_win_rate"] - data["ap_win_rate"]
    
    data.drop(columns=['home_pitcher', 'away_pitcher'], inplace=True)
    return data
...
def ratio_transform(data):
    for col in data.columns:
        ...
        if col[:4] == "home" and ( corsp_away in data.columns):
...
def gen_submission_csv(y_preds):
    dict = {
        "id": [],
        "home_team_win": []
    }
